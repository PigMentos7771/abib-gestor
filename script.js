...
let examesMedicosList = []; // Exames médicos ocupacionais (admissional, periódico, etc.)
// Configurações padrão do painel
let configGerais = {
    cnpjExameTrabalho: '63.098.051/0001-18',
    diasUrgente: 7,
    diasAtencao: 15,
    diasAsoUrgente: 3,
    diasAsoAtencao: 7,
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    (function() {
        var fromViewport = window.innerWidth - document.documentElement.clientWidth;
        var div = document.createElement('div');
        div.style.cssText = 'width:100px;height:100px;overflow:scroll;position:absolute;top:-9999px;';
        document.body.appendChild(div);
        var fromDiv = div.offsetWidth - div.clientWidth;
        document.body.removeChild(div);
        var scrollW = Math.max(fromViewport, fromDiv, 0);
    ...
    // A mágica acontece aqui: 'pt-BR' força o português
    ...
    const final = (dataFormatada.replace(/(?:^|\s|-)\S/g, function(a) { 
    return a.toUpperCase(); })).replace(/ De /g, " de ");

    dataDisplay.textContent = final;
    ...
    try {
        ...
        if (document.getElementById('cfg-cnpj-exame-trabalho')) {
            document.getElementById('cfg-cnpj-exame-trabalho').value = configGerais.cnpjExameTrabalho || '63.098.051/0001-18';
        }
...
let pontoSnapshotAssinatura = '';

function _assinarEstadoPontoAtual() {
    const idFunc = document.getElementById('ponto-select-func') ? document.getElementById('ponto-select-func').value : '';
    ...
    if (Array.isArray(folhaAtual) && folhaAtual.length > 0) {
        for (let d = 1; d < folhaAtual.length; d++) {
            const regBase = folhaAtual[d] || {};
            const e1 = document.getElementById(`p_${d}_e1`) ? document.getElementById(`p_${d}_e1`).value : (regBase.e1 || '');
            const s1 = document.getElementById(`p_${d}_s1`) ? document.getElementById(`p_${d}_s1`).value : (regBase.s1 || '');
            const e2 = document.getElementById(`p_${d}_e2`) ? document.getElementById(`p_${d}_e2`).value : (regBase.e2 || '');
            const s2 = document.getElementById(`p_${d}_s2`) ? document.getElementById(`p_${d}_s2`).value : (regBase.s2 || '');
            const folga = document.getElementById(`p_${d}_folga`) ? !!document.getElementById(`p_${d}_folga`).checked : !!regBase.isFolga;
            const abonado = document.getElementById(`p_${d}_abonar`) ? !!document.getElementById(`p_${d}_abonar`).checked : !!regBase.isAbonado;
            const obs = (regBase.observacao || '');

            linhas.push([d, e1 || '', s1 || '', e2 || '', s2 || '', folga ? 1 : 0, abonado ? 1 : 0, obs].join('|'));
        }
    }
...
function _atualizarSnapshotPonto() {
    pontoSnapshotAssinatura = _assinarEstadoPontoAtual();
}

function _pontoTemAlteracoesNaoSalvas() {
    const viewPontoAtiva = !!(document.getElementById('view-ponto') && document.getElementById('view-ponto').classList.contains('active'));
    const edicaoPontoAberta = !!(document.getElementById('ponto-edicao-container') && document.getElementById('ponto-edicao-container').style.display !== 'none');
    const assinaturaAtual = _assinarEstadoPontoAtual();
    const alteracaoRealDetectada = !!(assinaturaAtual && pontoSnapshotAssinatura && assinaturaAtual !== pontoSnapshotAssinatura);
    return (viewPontoAtiva || edicaoPontoAberta) && (pontoFoiAlterado || alteracaoRealDetectada);
}

async function checarAlteracoesNaoSalvasPonto() {
    if (_pontoTemAlteracoesNaoSalvas()) {
...
function showConfirmPontoSalvarESair(message) {
    return new Promise((resolve) => {
        ...
        const btnExtra = document.getElementById('custom-confirm-btn-extra');

        msgEl.textContent = message;
        btnOk.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Salvar e sair';
        btnCancel.textContent = 'Cancelar';
        btnExtra.classList.remove('hidden');
        btnExtra.textContent = 'Sair sem salvar';
        modal.classList.remove('hidden');

        const handleOk = () => { cleanup(); resolve('save'); };
        const handleCancel = () => { cleanup(); resolve('cancel'); };
        const handleExtra = () => { cleanup(); resolve('discard'); };

        const cleanup = () => {
            btnOk.removeEventListener('click', handleOk);
            btnCancel.removeEventListener('click', handleCancel);
            btnExtra.removeEventListener('click', handleExtra);
        ...
        btnExtra.addEventListener('click', handleExtra);
    });
}
...
// Ordem das abas no menu (exceto configuracoes, que nunca se oculta)
const ABAS_MENU_ORDEM = ['dashboard', 'pendencias', 'exames', 'gastos', 'funcionarios', 'desligamentos', 'ponto', 'whatsapp', 'mapa', 'historico'];

function tabEstaVisivel(tabId) {
    if (tabId === 'configuracoes') return true;
    const vis = configGerais.visibilidadeAbas;
    if (!vis || typeof vis !== 'object') return true;
    return vis[tabId] !== false;
}

function primeiraAbaVisivel() {
    for (let i = 0; i < ABAS_MENU_ORDEM.length; i++) {
        const id = ABAS_MENU_ORDEM[i];
        if (tabEstaVisivel(id)) return id;
    }
    return 'configuracoes';
}

function sincronizarFormVisibilidadeAbas() {
    ABAS_MENU_ORDEM.forEach(function(id) {
        const el = document.getElementById('cfg-aba-' + id);
        if (el) el.checked = tabEstaVisivel(id);
    });
}

function aplicarVisibilidadeAbasNoMenu() {
    document.querySelectorAll('.main-menu .menu-item[data-tab]').forEach(function(el) {
        const tab = el.getAttribute('data-tab');
        if (tab === 'configuracoes') {
            el.style.display = '';
            return;
        }
        el.style.display = tabEstaVisivel(tab) ? '' : 'none';
    });
    const sec = document.querySelector('.view-section.active');
    if (sec && sec.id && sec.id.startsWith('view-')) {
        const tid = sec.id.replace('view-', '');
        if (tid !== 'configuracoes' && !tabEstaVisivel(tid)) {
            switchTab(primeiraAbaVisivel(), null, { bypassAbasOcultas: true });
        }
    }
}

async function switchTab(tabId, editId = null, opts = {}) {
    if (!opts.bypassAbasOcultas && tabId !== 'configuracoes' && !tabEstaVisivel(tabId)) {
        showToast('Esta aba está oculta nas configurações. Reative-a em Configurações do sistema.', 'warning');
        await switchTab(primeiraAbaVisivel(), null, { bypassAbasOcultas: true });
        return;
    }
    ...
    if (tabId === 'exames') {
        if (typeof renderExamesMedicos === 'function') renderExamesMedicos();
    }
    if (tabId === 'mapa') {
        mapaInit();
    }
    if (tabId === 'configuracoes') {
        ...
        document.getElementById('cfg-pdf-ocultar-saldos').checked = !!configGerais.pdfOcultarSaldos;
        if (document.getElementById('cfg-pdf-ocultar-timestamp')) {
            document.getElementById('cfg-pdf-ocultar-timestamp').checked = !!configGerais.pdfOcultarTimestamp;
        }
        if (document.getElementById('cfg-cnpj-exame-trabalho')) {
            document.getElementById('cfg-cnpj-exame-trabalho').value = configGerais.cnpjExameTrabalho || '63.098.051/0001-18';
...
function atualizarVariaveisComData(data) {
    ...
    examesMedicosList = data.examesMedicos || [];
...
/**
 * Efetiva automaticamente o aviso prévio vencido.
 * Regra: ao chegar (ou passar) a data final do aviso, funcionário vira desligado.
 */
function autoEfetivarAvisosPreviosVencidos() {
    if (!Array.isArray(funcionariosList) || funcionariosList.length === 0) return 0;

    const hoje = moment().startOf('day');
    let alterados = 0;

    funcionariosList.forEach(f => {
        if (!f || !f.emAvisoPrevio || !f.dataFimAviso) return;
        const fimAviso = moment(f.dataFimAviso).startOf('day');
        if (!fimAviso.isValid()) return;
        if (!hoje.isSameOrAfter(fimAviso)) return;

        f.emAvisoPrevio = false;
        f.desligado = true;
        // Usa o último dia de aviso como data oficial do desligamento.
        f.dataDesligamento = fimAviso.format('YYYY-MM-DD');
        alterados++;
    });
...
function reRenderizarTelasAtivas() {
    ...
    if (document.getElementById('view-exames') && document.getElementById('view-exames').classList.contains('active')) {
        if (typeof renderExamesMedicos === 'function') renderExamesMedicos();
    }
    ...
    source.addEventListener('put', (e) => {
        try {
            ...
            if (parsed.path === "/") {
                ...
                _clearPontoAcumuladoFallbackCache();
                _clearPontoSaldoFallbackUi();
...
function invalidarCachePontoSplit(keyDb) {
    ...
    const parsed = _parsePontoFirebaseKey(cleanKey);
    if (parsed) {
        _clearPontoAcumuladoFallbackCache(parsed.idFunc);
        _clearPontoSaldoFallbackUi(parsed.idFunc);
    }
}

function reRenderizarPontoTelasAtivas() {
    if (_reRenderPontoTelasTimer) clearTimeout(_reRenderPontoTelasTimer);
    _reRenderPontoTelasTimer = setTimeout(() => {
        _reRenderPontoTelasTimer = null;
...
async function salvarDadosNuvem() {
    ...
    const payload = {
        ...
        examesMedicos: examesMedicosList,
...
function salvarCFG(e) {
    ...
    const cnpjExameTrabalho = document.getElementById('cfg-cnpj-exame-trabalho')
        ? document.getElementById('cfg-cnpj-exame-trabalho').value.trim()
        : '';
    ...
    configGerais.cnpjExameTrabalho = cnpjExameTrabalho || '63.098.051/0001-18';
    ...
    configGerais.visibilidadeAbas = {};
    ABAS_MENU_ORDEM.forEach(function(id) {
        const el = document.getElementById('cfg-aba-' + id);
        configGerais.visibilidadeAbas[id] = el ? !!el.checked : true;
    });
...
function direcionarEdicao(id) {
    ...
    if (prazo.tipoCod === 'experiencia') {
        ...
        if (func) {
            switchTab('funcionarios', func.idFunc, { bypassAbasOcultas: true });
            return;
        }
    }
    ...
    if (['rescisao', 'aso', 'fgts'].includes(prazo.tipoCod)) {
        ...
        if (func) {
            switchTab('desligamentos', func.idFunc, { bypassAbasOcultas: true });
            return;
        }
    }
...
function renderFuncionarios() {
    ...
    const showDesligados = document.getElementById('show-desligados-funcionarios')?.checked ?? false;
    ...
    let funcsExibidos = funcionariosList.filter(f => {
        if (!showDesligados && f.desligado) return false;
...
function exportarCSVFuncionarios() {
    ...
    const showDesligados = document.getElementById('show-desligados-funcionarios')?.checked ?? false;
    const listaParaExportar = showDesligados
        ? funcionariosList
        : funcionariosList.filter(f => !f.desligado);

    if (!listaParaExportar.length) {
        showToast('Não há funcionários ativos para exportar. Marque "Mostrar desligados" na lista para incluir quem já saiu.', "warning");
        return;
    }

    let dadosExportacao = listaParaExportar.map(f => {
        ...
        return {
            "NOME": f.nome || "",
            "CPF": f.cpf || "",
            "DATA_NASCIMENTO": f.dataNascimento ? moment(f.dataNascimento).format('DD/MM/YYYY') : "",
            "FUNCAO": f.funcao || "",
            "UNIDADE": f.unidade || "",
...
let pontoCache = {};
/** idFunc -> { 'YYYY-MM': number|null } — evita N×24 buscas repetidas no grid de ponto */
let pontoAcumuladoFallbackCache = {};
/** idFunc -> { 'YYYY-MM': number|null } — saldo “Banco acumulado” vindo do histórico (preenchido em background) */
let pontoSaldoFallbackUi = {};
let _renderListaPontoGen = 0;
let _renderListaPontoSearchTimer = null;
let _reRenderPontoTelasTimer = null;

function _clearPontoAcumuladoFallbackCache(idFunc) {
    if (idFunc) delete pontoAcumuladoFallbackCache[idFunc];
    else pontoAcumuladoFallbackCache = {};
}

function _clearPontoSaldoFallbackUi(idFunc) {
    if (idFunc) delete pontoSaldoFallbackUi[idFunc];
    else pontoSaldoFallbackUi = {};
}

function _parsePontoFirebaseKey(cleanKey) {
    const m = String(cleanKey).match(/^(.+)_(\d{4}-\d{2})$/);
    return m ? { idFunc: m[1], mesAno: m[2] } : null;
}

function invalidarCachePonto(idFunc, mesAno) {
    delete pontoCache[`${idFunc}_${mesAno}`];
    _clearPontoAcumuladoFallbackCache(idFunc);
    _clearPontoSaldoFallbackUi(idFunc);
}
...
async function _obterUltimoAcumuladoAteMes(idFunc, mesAno, limiteMeses = 24) {
    if (!pontoAcumuladoFallbackCache[idFunc]) pontoAcumuladoFallbackCache[idFunc] = {};
    const row = pontoAcumuladoFallbackCache[idFunc];
    if (Object.prototype.hasOwnProperty.call(row, mesAno)) {
        return row[mesAno];
    }
    const base = moment(mesAno, 'YYYY-MM');
    let out = null;
    // Um fetch por vez (evita centenas de GET paralelos → ERR_INSUFFICIENT_RESOURCES)
    for (let i = 1; i <= limiteMeses; i++) {
        const ref = base.clone().subtract(i, 'months').format('YYYY-MM');
        const db = await fetchPontoMes(idFunc, ref);
        if (!db) continue;
        if (db.fechamentoAcumulado !== undefined && db.fechamentoAcumulado !== null) {
            out = Number(db.fechamentoAcumulado) || 0;
            break;
        }
        if (db.fechamentoSaldoLiquido !== undefined && db.fechamentoSaldoLiquido !== null) {
            out = Number(db.fechamentoSaldoLiquido) || 0;
            break;
        }
    }
    row[mesAno] = out;
...
function _htmlSaldoLinhaPontoCard(f, mesAno, exibeSaldoHtml, saldoFallbackMapLocal) {
    if (exibeSaldoHtml && exibeSaldoHtml.trim()) return exibeSaldoHtml;
    let val;
    if (saldoFallbackMapLocal && Object.prototype.hasOwnProperty.call(saldoFallbackMapLocal, f.idFunc)) {
        val = saldoFallbackMapLocal[f.idFunc];
    } else {
        const byId = pontoSaldoFallbackUi[f.idFunc];
        if (byId && Object.prototype.hasOwnProperty.call(byId, mesAno)) val = byId[mesAno];
    }
    if (val === undefined || val === null) return '';
    const cor = val > 0 ? 'var(--success)' : (val < 0 ? 'var(--danger)' : 'var(--text-light)');
    return `<div style="font-size: 0.75rem; margin-top: 4px; color: ${cor}; font-weight: 600;"><i class="fa-solid fa-scale-balanced"></i> Banco Acumulado: ${formatMinutes(val)}</div>`;
}

function _renderListaPontoMontarCardsFinais(gridEl, keysOrd, sectionEls, agrupados, mesAno, diasNoMes, showVazio, showParcial, showCompleto, showConferido, emptyState, saldoFallbackMapLocal) {
    for (let u of keysOrd) {
        ...
        for (let f of funcsDaUnidade) {
            ...
            const saldoLinha = _htmlSaldoLinhaPontoCard(f, mesAno, exibeSaldoHtml, saldoFallbackMapLocal);
            card.innerHTML = _pontoDenso
                ? `<span class="ponto-card-nome">${f.nome}</span><span class="ponto-card-status" style="font-size:0.68rem;color:var(--text-light);">${f.unidade || ''}</span>${saldoLinha}`
                : `<span class="ponto-card-nome">${f.nome}</span><span class="ponto-card-status">${statusText}</span>${saldoLinha}${dl}`;
...
async function renderListaPonto() {
    ...
    for (let u of keysOrd) {
        ...
        for (let f of funcsDaUnidade) {
            ...
            if (pontoCache.hasOwnProperty(cacheKey)) {
                ...
                const saldoLinhaPass1 = _htmlSaldoLinhaPontoCard(f, mesAno, exibeSaldoHtml, null);
                card.innerHTML = _pontoDenso
    ...
    const gen = ++_renderListaPontoGen;
    ...
    if (funcsParaBuscar.length > 0) {
        ...
        try {
            ...
            if (gen !== _renderListaPontoGen) return;
            if (resp.ok) {
    ...
    // Cards com status imediato; saldo histórico (“Banco acumulado”) preenche em background
    const funcsSemAcumulado = ativos.filter(f => {
        const db = pontoCache[`${f.idFunc}_${mesAno}`];
        return !db || (db.fechamentoAcumulado === undefined || db.fechamentoAcumulado === null);
    });
    _renderListaPontoMontarCardsFinais(gridEl, keysOrd, sectionEls, agrupados, mesAno, diasNoMes, showVazio, showParcial, showCompleto, showConferido, emptyState, {});

    const funcsSemAcumuladoBg = funcsSemAcumulado.filter(f => {
        const o = pontoSaldoFallbackUi[f.idFunc];
        return !(o && Object.prototype.hasOwnProperty.call(o, mesAno));
    });
    const gFb = gen;
    const _FALLBACK_LOTE = 3;
    if (funcsSemAcumuladoBg.length > 0) {
        void (async () => {
            const saldoFallbackMap = {};
            for (let i = 0; i < funcsSemAcumuladoBg.length; i += _FALLBACK_LOTE) {
                if (gFb !== _renderListaPontoGen) return;
                const lote = funcsSemAcumuladoBg.slice(i, i + _FALLBACK_LOTE);
                await Promise.all(lote.map(async (f) => {
                    const val = await _obterUltimoAcumuladoAteMes(f.idFunc, mesAno);
                    if (val !== null && val !== undefined) saldoFallbackMap[f.idFunc] = val;
                }));
            }
            if (gFb !== _renderListaPontoGen) return;
            for (const f of funcsSemAcumuladoBg) {
                if (!pontoSaldoFallbackUi[f.idFunc]) pontoSaldoFallbackUi[f.idFunc] = {};
                pontoSaldoFallbackUi[f.idFunc][mesAno] = (saldoFallbackMap[f.idFunc] !== undefined && saldoFallbackMap[f.idFunc] !== null)
                    ? saldoFallbackMap[f.idFunc]
                    : null;
            }
            if (gFb !== _renderListaPontoGen) return;
            renderListaPonto();
...
function renderListaPontoDebounced() {
    if (_renderListaPontoSearchTimer) clearTimeout(_renderListaPontoSearchTimer);
    _renderListaPontoSearchTimer = setTimeout(() => {
        _renderListaPontoSearchTimer = null;
        renderListaPonto();
    }, 300);
}
...
async function voltarListaPonto() {
    if (_pontoTemAlteracoesNaoSalvas()) {
        const acao = await showConfirmPontoSalvarESair(
            "Atenção: Existem alterações não gravadas nesta Folha de Ponto.\n\n" +
            "Você pode salvar agora antes de sair, descartar as alterações ou cancelar."
        );
        if (acao === 'cancel') return;
        if (acao === 'save') {
            await salvarBancoAtual();
            if (_pontoTemAlteracoesNaoSalvas()) return; // Se falhou salvar, não sai.
        }
    }
...
async function renderFolhaPonto() {
    ...
    await atualizarTotalizadoresPonto(idFunc, mesAno);
    await corrigirFechamentoAcumuladoPontoSeNecessario(idFunc, mesAno, dadosSalvos);
    _atualizarSnapshotPonto();
    window.scrollTo(0, preservedScroll);
}

async function corrigirFechamentoAcumuladoPontoSeNecessario(idFunc, mesAno, dadosSalvos) {
    if (!dadosSalvos || !idFunc || !mesAno) return;

    const esperado = (cacheSaldoAnterior || 0) + (saldoMensalLiquido || 0);
    const atual = Number(dadosSalvos.fechamentoAcumulado);
    if (Number.isFinite(atual) && atual === esperado) return;
    ...
    try {
        await fetch(`${FIREBASE_URL}pontos/${idFunc}_${mesAno}.json`, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                fechamentoSaldoLiquido: saldoMensalLiquido || 0,
                fechamentoAcumulado: esperado
            })
        });
        if (pontoCache && pontoCache[`${idFunc}_${mesAno}`]) {
            pontoCache[`${idFunc}_${mesAno}`].fechamentoSaldoLiquido = saldoMensalLiquido || 0;
            pontoCache[`${idFunc}_${mesAno}`].fechamentoAcumulado = esperado;
        }
    } catch (e) {
        console.warn('Falha ao autocorrigir fechamentoAcumulado', e);
    }
}
...
async function fetchPontoMes(idFunc, mesAno) {
    ...
    try {
        ...
        // Falha de rede / limite do browser: não cachear null para permitir nova tentativa
        return null;
    }
...
async function salvarBancoAtual() {
    ...
    const evt = (typeof event !== 'undefined' && event) ? event : null;
...
const OCR_MATCH_SCORE_MIN = 0.92;
...
function _ocrTokenize(str) {
    return _ocrNorm(str).split(' ').filter(Boolean);
}

function _ocrNameScore(nomeLido, nomeFuncionario) {
    const lido = _ocrNorm(nomeLido);
    const base = _ocrNorm(nomeFuncionario);
    if (!lido || !base) return 0;
    if (lido === base) return 1;

    const tLido = _ocrTokenize(lido);
    const tBase = _ocrTokenize(base);
    if (!tLido.length || !tBase.length) return 0;

    const setBase = new Set(tBase);
    let hits = 0;
    tLido.forEach(t => { if (setBase.has(t)) hits++; });
    const tokenRatio = hits / Math.max(tLido.length, tBase.length);

    const firstL = tLido[0] || '';
    const lastL = tLido[tLido.length - 1] || '';
    const firstB = tBase[0] || '';
    const lastB = tBase[tBase.length - 1] || '';
    const firstLastBoost = (firstL && lastL && firstL === firstB && lastL === lastB) ? 0.2 : 0;
    const includeBoost = (base.includes(lido) || lido.includes(base)) ? 0.08 : 0;

    const fuzzy = Math.min(1, _fuzzyScore(base, lido) / 3);
    return Math.min(1, (tokenRatio * 0.65) + (fuzzy * 0.27) + firstLastBoost + includeBoost);
}

function _ocrInferirUnidadePorArquivo(fileItem) {
    if (!fileItem || !fileItem.filename) return '';
    const nomeArq = _ocrNorm(fileItem.filename.replace(/\.[^.]+$/, ''));
    if (!nomeArq) return '';

    const unidades = [...new Set(funcionariosList.map(f => _ocrNorm(f.unidade)).filter(Boolean))];
    let melhor = '';
    let melhorLen = 0;
    unidades.forEach(u => {
        if (u.length < 4) return;
        if (nomeArq.includes(u) && u.length > melhorLen) {
            melhor = u;
            melhorLen = u.length;
        }
    });
...
function _ocrRegistrarAuditoria(evt) {
    const registro = {
        at: moment().format('YYYY-MM-DD HH:mm:ss'),
        ...evt
    };
    ocrAuditLog.push(registro);
    if (ocrAuditLog.length > 400) ocrAuditLog = ocrAuditLog.slice(-400);
    try { salvarFilaOcrStorage(); } catch (e) {}
}

function _ocrDescreverCandidatos(cands) {
    if (!Array.isArray(cands) || cands.length === 0) return '';
    return cands.slice(0, 3).map(c => `${c.nome} (${Math.round(c.score * 100)}%)`).join(' | ');
}

function _ocrErroComCodigo(msg, code) {
    const err = new Error(msg);
    err.ocrCode = code;
    return err;
}

function _ocrPreencherSugestao(idFunc, nome) {
    const inpBusca = document.getElementById('ocr-vincular-search');
    const inpTarget = document.getElementById('ocr-vincular-target-func-id');
    const lista = document.getElementById('ocr-vincular-list');
    if (inpBusca) inpBusca.value = nome || '';
    if (inpTarget) inpTarget.value = idFunc || '';
    if (lista) lista.style.display = 'none';
}

function renderOcrSugestoesIA(missingCard) {
    const card = document.getElementById('ocr-sugestoes-ia-card');
    const lista = document.getElementById('ocr-sugestoes-ia-lista');
    if (!card || !lista) return;
    lista.innerHTML = '';

    const cands = (missingCard && missingCard.matchInfo && Array.isArray(missingCard.matchInfo.candidatos))
        ? missingCard.matchInfo.candidatos.slice(0, 3)
        : [];
    ...
    cands.forEach(c => {
        ...
        btn.type = 'button';
        btn.className = 'ocr-sugestao-btn';
        btn.textContent = `${c.nome} (${Math.round((c.score || 0) * 100)}%)`;
        btn.title = `${c.nome} (${c.unidade || 'Sem unidade'})`;
        btn.onclick = function () {
            _ocrPreencherSugestao(c.idFunc, c.nome);
        };
...
async function processarFilaOcrWorker() {
    ...
    try {
        ...
        if (code === 'RATE_LIMIT' || code === 'SERVICE_UNAVAILABLE') {
            proximo.ocrRetryCount = (proximo.ocrRetryCount || 0) + 1;
            const delay = Math.min(OCR_COOLDOWN_MAX_MS, OCR_COOLDOWN_BASE_MS * Math.pow(2, proximo.ocrRetryCount - 1));
            const seg = Math.ceil(delay / 1000);
            proximo.status = 'pendente';
            proximo.msgRetorno = `${e.message || 'API indisponível no momento.'} Reprocessando em ${seg}s (tentativa ${proximo.ocrRetryCount}).`;
            salvarFilaOcrStorage();
            renderFilaOcr();
...
function _ocrContaConflitosSobrescrita(dbAtual, aiJSON) {
    let conflitos = 0;
    for (let d = 1; d <= 31; d++) {
        if (!aiJSON.dias || !aiJSON.dias[d.toString()]) continue;
        const info = aiJSON.dias[d.toString()];
        const novo = {
            e1: formatTimeVal(info.e1),
            s1: formatTimeVal(info.s1),
            e2: formatTimeVal(info.e2),
            s2: formatTimeVal(info.s2)
        };
        const atual = dbAtual[d] || {};
        const temAtual = !!(atual.e1 || atual.s1 || atual.e2 || atual.s2);
        const temNovo = !!(novo.e1 || novo.s1 || novo.e2 || novo.s2);
        if (!temAtual || !temNovo) continue;
        if ((novo.e1 && atual.e1 && novo.e1 !== atual.e1) ||
            (novo.s1 && atual.s1 && novo.s1 !== atual.s1) ||
            (novo.e2 && atual.e2 && novo.e2 !== atual.e2) ||
            (novo.s2 && atual.s2 && novo.s2 !== atual.s2)) {
            conflitos++;
        }
    }
    return conflitos;
...
async function _obterSaldoBaseAcumuladoPonto(idFunc, mesAno, dbAtual) {
    if (dbAtual && dbAtual.saldoAnteriorManual !== undefined && dbAtual.saldoAnteriorManual !== null) {
        return Number(dbAtual.saldoAnteriorManual) || 0;
    }

    const mesAnterior = moment(mesAno, 'YYYY-MM').subtract(1, 'months').format('YYYY-MM');
    const dbMesAnterior = await fetchPontoMes(idFunc, mesAnterior);
    if (dbMesAnterior && dbMesAnterior.fechamentoAcumulado !== undefined && dbMesAnterior.fechamentoAcumulado !== null) {
        return Number(dbMesAnterior.fechamentoAcumulado) || 0;
    }
    if (dbMesAnterior && dbMesAnterior.fechamentoSaldoLiquido !== undefined && dbMesAnterior.fechamentoSaldoLiquido !== null) {
        return Number(dbMesAnterior.fechamentoSaldoLiquido) || 0;
    }
    return 0;
}

async function validarEGravarPontoLote(aiArray, mesAno, fileItem) {
    ...
    const unidadeContexto = _ocrInferirUnidadePorArquivo(fileItem);
    const funcsAtivos = funcionariosList.filter(f => !f.desligado);
    const funcsEscopo = unidadeContexto
        ? funcsAtivos.filter(f => _ocrNorm(f.unidade) === unidadeContexto)
        : funcsAtivos;
    const funcsBase = funcsEscopo.length > 0 ? funcsEscopo : funcsAtivos;

    for (let aiJSON of aiArray) {
        let lido = aiJSON.nome_lido_cartao || "";
        if (lido.trim() === "") {
            aiJSON.nome_lido_cartao = "Ilegível / Sem Nome";
            aiJSON.matchInfo = { motivo: 'nome_ilegivel', candidatos: [] };
            pendentesVincularArray.push(aiJSON);
            continue;
        }

        const ranked = funcsBase
            .map(f => ({ func: f, score: _ocrNameScore(lido, f.nome) }))
            .filter(x => x.score > 0)
            .sort((a, b) => b.score - a.score);

        const top = ranked[0] || null;
        const top2 = ranked[1] || null;
        const topGap = top && top2 ? (top.score - top2.score) : 1;
        const passouScore = !!(top && top.score >= OCR_MATCH_SCORE_MIN);
        const passouGap = !!(top && topGap >= OCR_MATCH_TOP_GAP_MIN);

        if (!top || !passouScore || !passouGap) {
            aiJSON.matchInfo = {
                motivo: !top ? 'sem_candidato' : (!passouScore ? 'baixa_confianca' : 'ambiguo_top2'),
                unidadeContexto: unidadeContexto || '',
                candidatos: ranked.slice(0, 5).map(x => ({
                    idFunc: x.func.idFunc,
                    nome: x.func.nome,
                    unidade: x.func.unidade || '',
                    score: x.score
                }))
            };
            pendentesVincularArray.push(aiJSON);
            _ocrRegistrarAuditoria({
                tipo: 'ocr_match_pendente',
                arquivo: fileItem ? fileItem.filename : '',
                nomeLido: lido,
                unidadeContexto: unidadeContexto || '',
                candidatos: _ocrDescreverCandidatos(aiJSON.matchInfo.candidatos)
            });
            continue;
        }

        let funcMatched = top.func;

        // Puxar banco atual daquela pessoa no mês
        let dbAtual = await fetchPontoMes(funcMatched.idFunc, mesAno) || {};

        const conflitos = _ocrContaConflitosSobrescrita(dbAtual, aiJSON);
        if (conflitos > 0) {
            const okSobrescrever = await showConfirm(
                `Detectei ${conflitos} conflito(s) no cartão de ${funcMatched.nome} (${mesAno}).\n` +
                `Os horários atuais serão substituídos nos dias conflitantes.\n\nDeseja continuar?`
            );
            if (!okSobrescrever) {
                mensagensErro.push(`Ignorado (sobrescrita cancelada): ${funcMatched.nome}`);
                _ocrRegistrarAuditoria({
                    tipo: 'ocr_overwrite_cancelado',
                    arquivo: fileItem ? fileItem.filename : '',
                    nomeLido: lido,
                    funcionario: funcMatched.nome,
                    conflitos
                });
                continue;
            }
        ...
        let saldoAntNuvem = await _obterSaldoBaseAcumuladoPonto(funcMatched.idFunc, mesAno, dbAtual);
        ...
        _ocrRegistrarAuditoria({
            tipo: 'ocr_aplicado_auto',
            arquivo: fileItem ? fileItem.filename : '',
            nomeLido: lido,
            funcionario: funcMatched.nome,
            scoreTop: Math.round(top.score * 100),
            scoreTop2: top2 ? Math.round(top2.score * 100) : null,
            unidadeContexto: unidadeContexto || '',
            conflitos
        });
    }
...
async function confirmarVinculacaoOcr() {
    ...
    try {
        ...
        if (conflitos > 0) {
            const okSobrescrever = await showConfirm(
                `Detectei ${conflitos} conflito(s) para ${targetFuncData.nome} (${mesAno}).\n` +
                `Deseja sobrescrever os horários existentes nesses dias?`
            );
            if (!okSobrescrever) {
                _ocrRegistrarAuditoria({
                    tipo: 'ocr_overwrite_cancelado_manual',
                    arquivo: itemQueue.filename || '',
                    nomeLido: aiJSON.nome_lido_cartao || '',
                    funcionario: targetFuncData.nome,
                    conflitos
                });
                showToast("Vinculação cancelada para evitar sobrescrita.", "warning");
                return;
            }
        }
        ...
        dbAtual.fechamentoSaldoLiquido = somatorioL;
        const saldoBaseAcumulado = await _obterSaldoBaseAcumuladoPonto(targetFuncData.idFunc, mesAno, dbAtual);
        dbAtual.fechamentoAcumulado = saldoBaseAcumulado + somatorioL;
...
function relGetPeriodo() {
    ...
    // Convertemos o objeto moment para uma data nativa do JS
    const dataNativa = m.toDate();

    // Usamos o formatador nativo do navegador para PT-BR
    const nomeMesExtenso = dataNativa.toLocaleString('pt-BR', { month: 'long' });
    const ano = dataNativa.getFullYear();

    return {
        mesAno:     m.format('YYYY-MM'),
        nomeMes:    (nomeMesExtenso.charAt(0).toUpperCase() + nomeMesExtenso.slice(1)) + '/' + ano,
...
// Carrega jsPDF standalone + AutoTable + fonte Inter dinamicamente
function _carregarAutoTable() {
    return new Promise((resolve, reject) => {
        ...
        // Resolve a URL real do TTF via Google Fonts CSS (user-agent TTF)
        const resolverTTF = (family, weight) =>
            fetch(`https://fonts.googleapis.com/css2?family=${family}:wght@${weight}`, {
                headers: { 'User-Agent': 'Mozilla/5.0 (compatible; MSIE 9.0)' }
            })
            .then(r => r.text())
            .then(css => {
                const match = css.match(/src:\s*url\(([^)]+\.ttf)\)/);
                if (!match) throw new Error('TTF não encontrado no CSS');
                return match[1];
            })
            .then(url => ttfParaBase64(url));

        (window.jspdf ? Promise.resolve() : carregarScript(jspdfUrl))
            .then(() => carregarScript(autotableUrl))
            .then(() => Promise.all([resolverTTF('Inter', 400), resolverTTF('Inter', 700)]))
            .then(([regular, bold]) => {
                const { jsPDF } = window.jspdf;
                jsPDF.API.events.push(['addFonts', function () {
                    this.addFileToVFS('Inter-Regular.ttf', regular);
                    this.addFileToVFS('Inter-Bold.ttf', bold);
                    this.addFont('Inter-Regular.ttf', 'Inter', 'normal');
                    this.addFont('Inter-Bold.ttf', 'Inter', 'bold');
                }]);
                window._pdfFonte = 'Inter';
                window._pdfFonteOk = true;
                resolve();
            })
            .catch(() => {
                // Fonte não carregou — usa helvetica sem quebrar
                window._pdfFonte = 'helvetica';
                window._pdfFonteOk = true;
                resolve();
            });
    });
...
// Retorna a fonte ativa (Inter se carregou, helvetica como fallback)
function _getFonte() { return window._pdfFonte || 'helvetica'; }

// Helpers de PDF vetorial
function _pdfHeader(doc, titulo, subtitulo, pdfW) {
    doc.setFillColor(67, 97, 238);
    doc.roundedRect(10, 10, pdfW - 20, 26, 5, 5, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(18);
    doc.setFont(_getFonte(), 'bold');
    doc.text(titulo, 16, 23);
    doc.setFontSize(9);
    doc.setFont(_getFonte(), 'normal');
    doc.text(subtitulo, 16, 28);
    return 42;
}

function _pdfCards(doc, cards, y, pdfW) {
    const total = cards.length;
    const totalW = pdfW - 20;
    const gap = 4;
    // Card 0 (valor total) recebe 35% da largura, os demais dividem o restante
    const card0W = totalW * 0.35;
    const restW  = (totalW - card0W - gap * (total - 1)) / (total - 1);
    const cardH  = 18;

    cards.forEach((c, i) => {
        const cardW = i === 0 ? card0W : restW;
        const x = i === 0 ? 10 : 10 + card0W + gap + (i - 1) * (restW + gap);
        const textX = x + cardW / 2;
        const textY = y + 10;
        const valor = String(c.num);
        doc.setFillColor(240, 242, 245);
        doc.roundedRect(x, y, cardW, cardH, 2, 2, 'F');
        doc.setTextColor(...(c.cor || [67, 97, 238]));
        // Card 0 com fonte maior para o valor caber
        doc.setFontSize(20);
        doc.setFont(_getFonte(), 'bold');
        doc.setLineWidth(0.3); // Define a espessura do "negrito extra"
        doc.setDrawColor(67, 97, 238); // Mesma cor do texto
        doc.setFillColor(67, 97, 238);
        // Desenha o texto 3 vezes com deslocamentos de 0.05mm
        doc.text(valor, textX, textY, { align: 'center' });
        doc.text(valor, textX + 0.01, textY, { align: 'center' });
        doc.text(valor, textX, textY + 0.01, { align: 'center' });
...
function _pdfSecTitulo(doc, texto, y, pdfW) {
    doc.setDrawColor(67, 97, 238);
    doc.setLineWidth(0.5);
    doc.line(10, y + 5, pdfW - 10, y + 5);
    doc.setTextColor(30, 58, 138);
    doc.setFontSize(10);
    doc.setFont(_getFonte(), 'bold');
    doc.text(texto, 10, y + 3);
    return y + 9;
}

function _pdfTabela(doc, colunas, linhas, y, pdfW, opts) {
    if (!linhas.length) {
        doc.setTextColor(107, 114, 128);
        doc.setFontSize(9);
        doc.setFont(_getFonte(), 'normal');
        doc.text('Nenhum registro no período.', 10, y + 6);
        return y + 12;
    }
    doc.autoTable({
        startY: y,
        ...
        body: linhas,
        theme: 'grid',
        styles: { fontSize: 8, cellPadding: 2.5, textColor: [17, 24, 39], lineColor: [229, 231, 235], lineWidth: 0.2, font: _getFonte() },
        headStyles: { fillColor: [238, 241, 253], textColor: [30, 58, 138], fontStyle: 'bold', fontSize: 8, font: _getFonte() },
...
function _pdfRodape(doc, pdfW, pdfH) {
    doc.setDrawColor(229, 231, 235);
    doc.setLineWidth(0.3);
    doc.line(10, pdfH - 12, pdfW - 10, pdfH - 12);
    doc.setTextColor(156, 163, 175);
    doc.setFontSize(7);
    doc.setFont(_getFonte(), 'normal');
    doc.text('MyABIB — Sistema de Gestão de RH', pdfW - 10, pdfH - 7, { align: 'right' });
}
...
async function baixarPDF() {
    ...
    try {
        await _carregarAutoTable();
    } catch(e) {
        restaurarBtn();
        showToast('Erro ao carregar biblioteca de PDF. Verifique sua conexão.', 'error');
        console.error('Erro ao carregar jsPDF/autoTable:', e);
        return;
    }
    ...
    try {
        ...
        if (tipoRelatorioAtual === 'gastos') {
            _baixarPDFGastos(doc, periodo, pdfW, pdfH);
        } else {
            _baixarPDFRH(doc, periodo, pdfW, pdfH);
        }
        ...
        for (let p = 1; p <= totalPags; p++) {
            doc.setPage(p);
            _pdfRodape(doc, pdfW, pdfH);
            if (totalPags > 1) {
                doc.setTextColor(156, 163, 175);
                doc.setFontSize(7);
                doc.text(`Página ${p} de ${totalPags}`, 10, pdfH - 7);
            }
        }
        ...
        console.error('Erro ao gerar PDF vetorial:', e);
    } finally {
        restaurarBtn();
    }
}

function _baixarPDFGastos(doc, periodo, pdfW, pdfH) {
    ...
    let y = _pdfHeader(doc, 'Relatório de Gastos',
        `${periodo.nomeMes}  ·  Gerado em ${moment().format('DD/MM/YYYY [às] HH:mm')}`, pdfW);

    y = _pdfCards(doc, [
        { num: fmtBRL(totalGeral), label: 'Total do Mês',   cor: [67, 97, 238] },
        { num: doMes.length,        label: 'Lançamentos',    cor: [67, 97, 238] },
        { num: linhasUnidade.length,label: 'Unidades',       cor: [67, 97, 238] },
        { num: linhasCat.length,    label: 'Categorias',     cor: [67, 97, 238] },
    ], y, pdfW);

    // Destaque maior lançamento
    if (maiorGasto) {
        doc.setFillColor(255, 247, 237);
        doc.setDrawColor(217, 119, 6);
        doc.setLineWidth(0.4);
        doc.roundedRect(10, y, pdfW - 20, 8, 1.5, 1.5, 'FD');
        doc.setTextColor(17, 24, 39);
        doc.setFontSize(8);
        // "Maior lançamento:" em negrito
        doc.setFont(_getFonte(), 'bold');
        doc.text('Maior lancamento:', 14, y + 5.2);
        const labelW = doc.getTextWidth('Maior lancamento: ');
        // restante em normal
        doc.setFont(_getFonte(), 'normal');
        const resto = `${maiorGasto.descricao || '—'} — ${fmtBRL(maiorGasto.valor)} (${maiorGasto.unidade || '—'})`;
        doc.text(resto, 14 + labelW, y + 5.2);
        y += 13;
    }

    if (relCheck('rel-check-por-unidade')) {
        y = _pdfSecTitulo(doc, 'Total por Unidade', y, pdfW);
        y = _pdfTabela(doc, ['Unidade', 'Lançamentos', 'Total (R$)'],
            linhasUnidade.map(([u, v]) => [u, v.qtd, fmtBRL(v.total)]), y, pdfW,
            { columnStyles: { 2: { halign: 'right' } } });
    }

    if (relCheck('rel-check-por-categoria')) {
        y = _pdfSecTitulo(doc, 'Total por Categoria', y, pdfW);
        y = _pdfTabela(doc, ['Categoria', 'Lançamentos', 'Total (R$)'],
            linhasCat.map(([cat, v]) => [cat, v.qtd, fmtBRL(v.total)]), y, pdfW,
            { columnStyles: { 2: { halign: 'right' } } });
    }

    if (relCheck('rel-check-lista-gastos')) {
        const agrupar = relCheck('rel-check-agrupar-unidade');

        if (agrupar) {
            const porUnidOrdenada = {};
            [...doMes].sort((a, b) => {
                const uA = (a.unidade || 'Sem Unidade').localeCompare(b.unidade || 'Sem Unidade');
                return uA !== 0 ? uA : (b.data || '').localeCompare(a.data || '');
            }).forEach(g => {
                const u = g.unidade || 'Sem Unidade';
                if (!porUnidOrdenada[u]) porUnidOrdenada[u] = [];
                porUnidOrdenada[u].push(g);
            });

            let primeiro = true;
            Object.entries(porUnidOrdenada).forEach(([unidade, gastos]) => {
                const totalUnid = gastos.reduce((acc, g) => acc + (parseFloat(g.valor) || 0), 0);
                if (primeiro) {
                    y = _pdfSecTitulo(doc, 'Lista Detalhada de Lançamentos', y, pdfW);
                    primeiro = false;
                }
                // Cabeçalho do grupo (unidade)
                if (y > pdfH - 40) { doc.addPage(); y = 15; }
                doc.setFillColor(238, 241, 253);
                doc.rect(10, y, pdfW - 20, 7, 'F');
                doc.setTextColor(30, 58, 138);
                doc.setFontSize(8.5);
                doc.setFont(_getFonte(), 'bold');
                doc.text(unidade, 13, y + 4.8);
                doc.text(fmtBRL(totalUnid), pdfW - 13, y + 4.8, { align: 'right' });
                y += 7;

                y = _pdfTabela(doc, ['Data', 'Descrição', 'Categoria', 'Valor (R$)'],
                    gastos.map(g => [fmt(g.data), g.descricao || '—', g.tipo || '—', fmtBRL(g.valor)]),
                    y, pdfW, {
                        columnStyles: { 0: { cellWidth: 22 }, 2: { cellWidth: 36 }, 3: { halign: 'right', cellWidth: 26 } },
                        margin: { left: 10, right: 10 }
                    });
                y += 2;
            });
            ...
            y = _pdfTabela(doc, ['Data', 'Descrição', 'Categoria', 'Unidade', 'Valor (R$)'],
                ordenados.map(g => [fmt(g.data), g.descricao || '—', g.tipo || '—', g.unidade || '—', fmtBRL(g.valor)]),
                y, pdfW, {
                    columnStyles: { 0: { cellWidth: 22 }, 4: { halign: 'right', cellWidth: 26 } }
                });
        }
    }

    // Total geral
    if (y > pdfH - 25) { doc.addPage(); y = 15; }
    doc.setDrawColor(67, 97, 238);
    doc.setLineWidth(0.5);
    doc.line(10, y, pdfW - 10, y);
    doc.setTextColor(30, 58, 138);
    doc.setFontSize(10);
    doc.setFont(_getFonte(), 'bold');
    doc.text(`Total Geral: ${fmtBRL(totalGeral)}`, pdfW - 10, y + 6, { align: 'right' });
}

function _baixarPDFRH(doc, periodo, pdfW, pdfH) {
    const fmt = d => d ? moment(d).format('DD/MM/YYYY') : '—';

    const ativos       = funcionariosList.filter(f => !f.desligado && !f.emAvisoPrevio);
    const admissoes    = funcionariosList.filter(f => (f.admissao || '').startsWith(periodo.mesAno));
    const desligados   = funcionariosList.filter(f => f.desligado && (f.dataDesligamento || '').startsWith(periodo.mesAno));
    const prazosVenc   = prazosList.filter(p => {
        const dv = p.dataVencimento || '';
        return dv >= periodo.inicio && dv <= periodo.fim && moment(dv).diff(moment().startOf('day'), 'days') < 0;
    });
    const pendAbertas  = pendenciasList.filter(p => !p.concluida);
    const pendConcluid = pendenciasList.filter(p => p.concluida && (p.dataConclusao || '').startsWith(periodo.mesAno));

    let y = _pdfHeader(doc, 'Relatório de RH',
        `${periodo.nomeMes}  ·  Gerado em ${moment().format('DD/MM/YYYY [às] HH:mm')}`, pdfW);

    y = _pdfCards(doc, [
        { num: ativos.length,       label: 'Funcionários Ativos',    cor: [67, 97, 238] },
        { num: admissoes.length,    label: 'Admissões no Mês',       cor: [67, 97, 238] },
        { num: desligados.length,   label: 'Desligamentos no Mês',   cor: [67, 97, 238] },
        { num: prazosVenc.length,   label: 'Prazos Vencidos',        cor: [67, 97, 238] },
        { num: pendAbertas.length,  label: 'Pendências Abertas',     cor: [67, 97, 238] },
    ], y, pdfW);

    if (relCheck('rel-check-admissoes')) {
        y = _pdfSecTitulo(doc, 'Admissões no Período', y, pdfW);
        y = _pdfTabela(doc, ['Nome', 'Função', 'Unidade', 'Data Admissão', 'Tipo Contrato'],
            admissoes.map(f => [f.nome, f.funcao || '—', f.unidade || '—', fmt(f.admissao),
                f.idPrazoVinculado ? 'Experiência' : 'Indeterminado']), y, pdfW);
    }
    if (relCheck('rel-check-desligamentos')) {
        y = _pdfSecTitulo(doc, 'Desligamentos no Período', y, pdfW);
        y = _pdfTabela(doc, ['Nome', 'Unidade', 'Data Desligamento', 'Motivo'],
            desligados.map(f => [f.nome, f.unidade || '—', fmt(f.dataDesligamento),
                f.motivoDesligamento || 'Não informado']), y, pdfW);
    }
    if (relCheck('rel-check-prazos')) {
        y = _pdfSecTitulo(doc, 'Prazos Vencidos no Período', y, pdfW);
        y = _pdfTabela(doc, ['Funcionário', 'Unidade', 'Tipo de Prazo', 'Data Vencimento'],
            prazosVenc.map(p => [p.nome, p.unidade || '—', p.tipo || p.tipoCod || '—',
                fmt(p.dataVencimento)]), y, pdfW);
    }
    if (relCheck('rel-check-pend-abertas')) {
        y = _pdfSecTitulo(doc, 'Pendências Abertas', y, pdfW);
        y = _pdfTabela(doc, ['Descrição', 'Categoria', 'Prioridade', 'Criado em', 'Vencimento'],
            pendAbertas.map(p => [p.descricao, p.categoria || '—', (p.prioridade || '—').toUpperCase(),
                fmt(p.dataCriacao), p.vencimento ? fmt(p.vencimento) : 'Não definido']), y, pdfW);
    }
    if (relCheck('rel-check-pend-concluidas')) {
        y = _pdfSecTitulo(doc, 'Pendências Concluídas no Período', y, pdfW);
        y = _pdfTabela(doc, ['Descrição', 'Categoria', 'Data Conclusão'],
            pendConcluid.map(p => [p.descricao, p.categoria || '—', fmt(p.dataConclusao)]), y, pdfW);
    }
}
...
function relCSS() {
    return {
        page:       'font-family:Inter,Helvetica,Arial,sans-serif;font-size:12px;color:#111827;',
...
function buildRelatorioGastos(periodo) {
    ...
    // Lista detalhada
    if (relCheck('rel-check-lista-gastos')) {
        const agruparPorUnidade = relCheck('rel-check-agrupar-unidade');

        if (agruparPorUnidade) {
            // Ordenação primária: unidade (A-Z); secundária: data (mais recente primeiro)
            const porUnidOrdenada = {};
            [...doMes]
                .sort((a, b) => {
                    const uA = (a.unidade || 'Sem Unidade').localeCompare(b.unidade || 'Sem Unidade');
            ...
            // Larguras fixas para alinhar colunas entre todos os grupos
            ...
            const ths = ['Data', 'Descrição', 'Categoria', 'Valor (R$)']
                .map(c => `<th style="${s.th}">${c}</th>`).join('');

            const entries = Object.entries(porUnidOrdenada);
            entries.forEach(([unidade, gastos], idx) => {
                const totalUnid = gastos.reduce((acc, g) => acc + (parseFloat(g.valor) || 0), 0);
                const trs = gastos.map((g, i) => {
                    const bg = i % 2 === 0 ? s.td : s.tdAlt;
                    return `<tr>
                        <td style="${bg}">${fmt(g.data)}</td>
                        <td style="${bg}">${esc(g.descricao || '—')}</td>
                        <td style="${bg}">${esc(g.tipo || '—')}</td>
                        <td style="${bg}text-align:right;">${fmtBRL(g.valor)}</td>
                    </tr>`;
                }).join('');

                // No primeiro grupo: incluir o título da seção junto para não separar na quebra de página
                const tituloSecao = idx === 0
                    ? `<div style="${s.secTit}margin-bottom:6px;">Lista Detalhada de Lançamentos</div>`
                    : '';

                html += `<div style="margin-top:${idx === 0 ? '18px' : '10px'};page-break-inside:avoid;">
                    ${tituloSecao}
                    <div style="border:1px solid #e5e7eb;border-radius:4px;overflow:hidden;">
                        <table style="${s.table}border-collapse:collapse;table-layout:fixed;">
                            ${colgroup}
                            <thead>
                                <tr>
                                    <th colspan="4" style="background:#eef1fd;color:#1e3a8a;padding:5px 8px;text-align:left;border-bottom:1px solid #d1d5db;font-size:11px;font-weight:bold;">
                                        📍 ${esc(unidade)}
                                        <span style="float:right;">${fmtBRL(totalUnid)}</span>
                                    </th>
                                </tr>
                                <tr>${ths}</tr>
                            </thead>
...
function baixarPDFBH() {
    ...
    // Salvar e remover limitações de scroll para captura completa
    const oldMaxHeight = area.style.maxHeight;
    const oldOverflow  = area.style.overflow;
    const oldOverflowY = area.style.overflowY;
    area.style.maxHeight = 'none';
    area.style.overflow  = 'visible';
    area.style.overflowY = 'visible';

    html2pdf().set(opt).from(area).save().then(() => {
        area.style.maxHeight = oldMaxHeight;
        area.style.overflow  = oldOverflow;
        area.style.overflowY = oldOverflowY;
    });
}
            ...
            html += `<div style="display:flex; gap:0.75rem; align-items:flex-start; padding:0.6rem 0.5rem;
                ...
                <div style="flex:1; min-width:0;">
                    ...
                    ${e.idFunc ? `<div style="font-size:0.73rem; color:var(--primary); margin-top:3px; cursor:pointer;"
                        onclick="switchTab('funcionarios', null, { bypassAbasOcultas: true }); setTimeout(()=>{ document.getElementById('search-funcionarios').value='${e.idFunc}'; renderFuncionarios(); }, 200);">
        ...
        mapaCarregarNotas().then(function() {
            mapaCarregarPosicoes().then(function(posicoesSalvas) {
                if (_mapa.nos && _mapa.nos.length > 0) {
                    ...
                    // Prioridade: posição salva no Firebase → posição já em memória (fallback
                    // contra race condition onde o save ainda não completou ao voltar para a aba).
                    _mapa.nos.forEach(function(n) {
                        const saved = posicoesSalvas && posicoesSalvas[n.id];
                        if (saved && saved.x !== undefined) {
                            n.x = saved.x; n.y = saved.y;
                            n._vx = saved.x; n._vy = saved.y;
                        }
                        // Se não tem posição salva mas já tem posição em memória, mantém (não zera)
                    });
                    ...
                    // Sem nós em memória: primeira carga real — reconstrói do zero
    ...
    corpo.innerHTML = `
        ...
        <div style="margin-top:1rem;display:flex;justify-content:flex-end;">
            <button class="btn-primary" style="font-size:0.82rem;height:34px;"
                onclick="mapaFecharSidebar(); switchTab('funcionarios','${esc(f.idFunc)}',{ bypassAbasOcultas: true })">
                <i class="fa-solid fa-pen"></i> Editar
            </button>
        </div>` : ''}
    `;
...
async function _agenteProcessarAcao(acao, textoOriginal) {
    ...
    switch (acao.acao) {
        ...
        case 'navegar': {
            const abasValidas = ['dashboard','pendencias','exames','gastos','funcionarios','desligamentos','ponto','whatsapp','mapa','historico','configuracoes'];
            if (acao.aba_destino && abasValidas.includes(acao.aba_destino)) {
                ...
                setTimeout(function(){ switchTab(acao.aba_destino, null, { bypassAbasOcultas: true }); }, 600);
            } else {
        ...
        case 'abrir_funcionario': {
            ...
            if (func) {
                ...
                setTimeout(function(){ switchTab('funcionarios', null, { bypassAbasOcultas: true }); setTimeout(function(){ abrirModalEditFunc(func.idFunc); }, 400); }, 600);
            } else {
